...
export class IMCComponent {
  ...
  constructor(private imcservice: ImcService) {
    this.imcservice.GetSexo(localStorage.getItem('token')!).subscribe(
      (data: any) => {
        data = 'femenino' ? (this.sexo = 'female') : (this.sexo = 'male');
      },
      (error: any) => {
  ...
  async CalcularIMC() {
    this.imcservice
      .cobinarLlmadosApi(
        Number.parseInt(this.IMC.altura),
        Number.parseInt(this.IMC.peso),
        Number.parseInt(this.IMC.edad),
        this.sexo
      )
      .subscribe({
        next: (data: any) => {
